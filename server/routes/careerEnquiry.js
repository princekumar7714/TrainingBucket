...
const filePath = join(__dirname, '..', 'data', 'career_enquiries.json');
...
router.post('/', (req, res, next) => {
  try {
    const err = validate(req.body || {}, {
      ...
      course: (v) => isRequired(v, 'Course'),
    });
    if (err) return apiError(res, err, 400);

    const { name, email, phone, course, branch } = req.body;
    const list = read();
    const entry = {
      id: String(Date.now()),
    ...
    list.push(entry);
    write(list);
    return apiSuccess(res, { id: entry.id }, 'Career enquiry submitted', 201);
  } catch (e) {
    next(e);
  }
});
